...
function handleToggleMenu(element) {
	...
	if (element.id === 'sidebar--toggle-open') {
		...
		sidebarToggle.forEach((item) => {
			...
			if (item.classList.value.includes('logo') || item.closest('.btn--2')) {
				item.style.padding = 0;
			}
		});
		sidebar.style.width = '240px';
		sidebar.querySelector('.sidebar--menu').style.padding = '75px 46px 180px 24px';
	} else {
		btnClose.style.display = 'none';
		btnOpen.style.display = 'block';
		sidebar.style.width = '80px';
		sidebar.querySelector('.sidebar--menu').style.padding = '75px 31px 170px 31px';
